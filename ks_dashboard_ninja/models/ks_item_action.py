...
class KsDashboardNinjaBoardItemAction(models.TransientModel):
    ...
    name = fields.Char()
    ks_dashboard_item = fields.Many2one("ks_dashboard_ninja.item", default=lambda self: self._context['ks_dashboard_item_id'])
    ...
    def action_item_move_copy_action(self):
        if self.ks_action == 'move':
            ...
            for id in self.ks_dashboard_ninja_ids.ids:
                ...
                self.ks_dashboard_item.sudo().copy({'ks_dashboard_ninja_board_id': id})
