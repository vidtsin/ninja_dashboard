...
class KsDashboardNinjaItems(models.Model):
    ...
    @api.multi
    @api.depends('ks_record_count_type', 'ks_model_id', 'ks_domain', 'ks_record_field','ks_item_end_date','ks_item_start_date')
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec):
        if ks_domain and "%DEPT" in ks_domain:
            ks_domain = ks_domain.replace("%DEPT", str(self.env.user.employee_ids.department_id.name))
...
# Pro Function
    ...
    @api.depends('ks_chart_measure_field', 'ks_chart_relation_groupby', 'ks_chart_date_groupby', 'ks_domain',
                 ...
                 'ks_goal_bar_line','ks_item_start_date','ks_item_end_date')
                    ...
                    if rec.ks_chart_groupby_type == 'relational_type' and rec.ks_chart_relation_groupby:
                        ...
                        if rec.ks_goal_enable and rec.ks_dashboard_item_type in ['ks_bar_chart',
                            ...
                            if selected_start_date and selected_end_date:
                                ksGoalDomain.extend([('ks_goal_date', '>=', selected_start_date),
                                                     ('ks_goal_date', '<=', selected_end_date)])

                            ks_goal_records = self.env['ks_dashboard_ninja.item_goal'].read_group(
                                ksGoalDomain, ['ks_goal_value','ks_goal_date'],
    ...
    @api.depends('ks_domain', 'ks_dashboard_item_type', 'ks_model_id', 'ks_sort_by_field', 'ks_sort_by_order',
                 'ks_record_data_limit', 'ks_list_view_fields', 'ks_list_view_group_fields', 'ks_list_view_type',
                 'ks_chart_relation_groupby','ks_chart_date_groupby')
    def ks_get_list_view_data(self):
        for rec in self:
            if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == 'ks_list_view' and rec.ks_model_id:
